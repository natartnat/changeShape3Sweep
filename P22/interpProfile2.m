function [shape] = interpProfile2(imgA,imgB,divide)
...
sAsim = size(psAsim,1); %point in A
sBsim = size(psBsim,1); %point in B
if sAsim < sBsim
...
[psAsim,iax] = dpsimplify(psA,1);
[psBsim,ibx] = dpsimplify(psB,1);
    ...
    for pi = 1:1:sA
        ...
        try
            ...
            elseif psA(pi,1) ~= psB(matching(pi),1)
                ...
                nY = interp1([psA(pi,1);psB(matching(pi),1)],[psA(pi,2);psB(matching(pi),2)],nX);
                new_out(pi,1) = nX;
                new_out(pi,2) = nY;
            %% x is not different
            else
                ...
                nY = interp1([psA(pi,2);psB(matching(pi),2)],[psA(pi,1);psB(matching(pi),1)],nX);
                new_out(pi,1) = nY;
                new_out(pi,2) = nX;
            end
